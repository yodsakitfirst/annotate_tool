...
def seed_assignment(runtime: Path) -> None:
    ...
    (root / "labels" / "a.txt").write_text("3 0.5 0.5 0.25 0.5\n", encoding="utf-8")
    ...
    repository.register_assignment("dataset-1", "Sample dataset", root)
...
def test_seeded_assignment_renders_review_controls(monkeypatch, tmp_path):
    ...
    assert not app.exception
    assert app.selectbox[0].value == "Sample dataset"
