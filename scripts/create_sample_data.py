...
def create_sample_project_archives(
    dataset_output_path: Path,
    catalog_output_path: Path,
) -> tuple[Path, Path]:
    dataset_output_path = dataset_output_path.resolve()
    catalog_output_path = catalog_output_path.resolve()
    ...
    with tempfile.TemporaryDirectory(prefix="annotation_sample_") as temporary:
        ...
        images = root / "images"
        labels = root / "labels"
        references = root / "catalog_references"
        images.mkdir()
        labels.mkdir()
        references.mkdir()
        ...
        (labels / "multiple_boxes.txt").write_text(
            "9999 0.234375 0.5078125 0.25 0.5625\n"
        ...
        (root / "data.yaml").write_text(
            yaml.safe_dump({"names": {17: "Legacy Product 17"}}),
            encoding="utf-8",
        )
        target_names = {3: "Product 03", 17: "Product 17", 20: "Product 20"}
        for class_id in target_names:
            _save_reference(references / f"{class_id}.jpg", class_id)

        with ZipFile(dataset_output_path, "w", compression=ZIP_DEFLATED) as archive:
            for path in sorted(root.rglob("*")):
                if path.is_file() and references not in path.parents:
                    archive.write(path, path.relative_to(root).as_posix())
        with ZipFile(catalog_output_path, "w", compression=ZIP_DEFLATED) as archive:
            archive.writestr(
                "catalog.yaml",
                yaml.safe_dump({"names": target_names}, sort_keys=True),
            )
            for path in sorted(references.iterdir()):
                archive.write(path, f"references/{path.name}")
    return dataset_output_path, catalog_output_path


def create_sample_archive(output_path: Path) -> Path:
    create_sample_project_archives(
        output_path,
        output_path.with_name("sample_reference_catalog.zip"),
    )
    return output_path.resolve()


if __name__ == "__main__":
    generated = create_sample_project_archives(
        Path("sample_data/sample_dataset.zip"),
        Path("sample_data/sample_reference_catalog.zip"),
    )
